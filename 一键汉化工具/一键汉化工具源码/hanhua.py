...
def modify_keys_json():
    ...
    try:
        ...
        # 检查并修改local字段
        if data.get("local") == "en":
            data["local"] = "zh"
            # 写回文件
            with open(keys_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            return True
        return False  # 已经是zh则无需修改

    except FileNotFoundError:
        messagebox.showerror("错误", f"未找到keys.json文件：{keys_path}")
        return False
    except json.JSONDecodeError:
        messagebox.showerror("错误", "JSON文件格式错误，请检查文件内容")
        return False
    except PermissionError:
        messagebox.showerror("错误", "没有文件写入权限，请用管理员权限运行")
        return False
...
def copy_localization_folder():
    ...
    try:
        ...
        # 修改keys.json文件
        if modify_keys_json():
            messagebox.showinfo("成功", "汉化完成！已成功修改语言设置")
        else:
            messagebox.showinfo("成功", "汉化完成！但语言设置无需修改")
    except Exception as e:
